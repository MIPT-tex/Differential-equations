...
\subsection{Понятие кольца. Пассмотрение пронятия многочленов}
\begin{definition}
...
\begin{enumerate}
    ...
    \item $\forall a \in K \exists -a\in K: a+(-a)=0\foralla\in K$
    \item $(a+b)\cdotc=ac+bc\;\;\;\forall a,b,c\in K$
...
\end{proposition}
\begin{proof}
$a\cdot 0+0=a\cdot 0 = a(0+0)\Rightarrowa\cdot 0 =0$; аналогично $0+0\cdot a=0\cdot a=(0+0)\cdot a =0\cdot a +0\cdot a \Rightarrow 0\cdot a =0$
\end{proof}
\begin{proposition}
Единица единственна
\end{proposition}
...
\end{proposition}
\begin{proof}
$ab=0, \;\;a\neq 0 \Rightarrow a^{-1}\cdot(ab)=a^{-1}\cdot 0=0=(a^{-1}a)\cdot b=1\cdot b =b \Rightarrowb=0$
\end{proof}
...
\begin{enumerate}
    ...
    $c_s=a_s+b_s,a_s=0,$ если $s>n$ и $b_s=0,$ если $s>m$\par Так определенное сложение многочленов коммутативно и ассоциативно.\parИмеется нулевой элемент $0=0\cdotx+\dots+0\cdot x^n,$ а также противополжный $\big(-P_n(x)\big)=(-a_0)+(-a_1)x++\dots+(-a^n)x^n$
...
\begin{itemize}
    ...
    \item Пусть $x\in \mathds{C}$. Значение многочлена $P_n(x)$ на $\mathds{C}$ определим как число $P_n(x)=a_0+a_1z+\dots+a_n^z^n\in \mathds{C}$.\par
    ...
    \item $(L_n(p)M_m(p))(f)=(a_0b_0p^0+(a_0b_1+a_1b_0)p+\dots+(\displaystyle\sum_{j=k+l}a_kb_l)p^j+\dots+a_nb_mp^{m+n})(f)=a_0b_0+(a_0b_1++a_1b_0)f'+\dots+(\displaystyle\sum_{j=k+l}a_kb_l)f^{(j)}+\dots+a_nb_mf^{(n+m)}= (a_0p^0+a_1p+\dots+a_np^n)\cdot(b_0f+b_1f'+\dots+b_mf^{(m)})=\;\;=L_n(p)\cdot (M_m(f))~-~$ опредеоление действия произведения операторов на множестве $\varPhi$. Так как $a_0b_0f++(a_0b_1+a_1b_0)f'+\dots+(\displaystyle\sum_{j=k+l}a_kb_l)f^{(j)}+\dots+ a_nb_mf^{(m+n)}=M_m(p)\cdot (a_0f+a_1f'+\dotsa_nf^{(n)})\Rightarrow (L_n(p)\cdot M_m(p))=\;\;=(M_m(p)\cdot L_n(p)) ~-~$ коммутативность.
    ...
    \begin{theorem}
    $$P_n(x)=a_nx^n+a_{n-1}x^{n-1}+\dots+a_0\in A(x),\;c\inA\Rightarrow\exists!Q_m(x), r\in \mathds{C}:P_n(x)=(x-c)Q_m(x)+r$$
    \end{theorem}
    \begin{theorem}
    ...
    \end{equation}
    Из $(\ref{3})$ следуетЮ что образом нуля кольца $K$ будет еуль $K$: $\varphi(a)=a'\inK'$ и $\varphi(0)=c',\;\;\varphi(a)=a'=\varphi(a+o)=\;\;\;=\varphi(a)+\varphi(0)=a'+c'\Rightarrow c'=0$\par
    ...
    $$\varphi(P_n(z)\cdotQ_m(z))=\varphi(a_0b_0+\displaystyle\sum_{j=k+l}a_kb_lz^j+a_nb_mz^{m+n})=(a_0b_0+\displaystyle\sum_{j=k+l}a_kb_lp^j+a_nb_mp^{m+n})=L_n(p)\cdot Q_m(p)(f)$$
...
\end{itemize}
\subsection{Линейные уравнения с потоянными коэффициентами}
Рассмотрим ДУ вида: $a_n y^{(n)}+a_{n-1}y^{(n-1)} +... +a_0y=0,\;\;\;a_n\neq 0, $ где $a_i=const\;\;\foralli=\overline{1,n}.$ Через введенный ранее дифференциальный оператор $L_n(p)=a_np^n+\dots + a_0p^0$ уравнение записывается в виде \begin{equation}\tag{2.1}
\label{2.1}
    L_n(p)(y(x))=0
\end{equation}
...
\end{proof}
\begin{theorem}
Система функций $P_n_1(x)e^{\lambda_1x},\dots,P_n_s(x)e^{\lambda_s x},$ где $P_n_i(x)$ является многочленом степени $n_i$, а все $\lambda_i\in \mathds{C}$ разные, является ЛНЗ.
\end{theorem}
\begin{proof}
...
Тогда $(k+1)-$я производная будет равна \begin{equation*}
    \frac{d^{k+1}y}{dx^{k+1}}=\frac{d}{dx}\Big(\frac{d^ky}{dx^k}\Big)=e^{-t}\frac{d}{dt}\Big(\frac{d^ky}{dx^k}\Big)=e^{-(k+1)t}\Big(\frac{d^{k+1}y}{dt^{k+1}}+(\alpha_1-k)\frac{d^ky}{dt^k}+\dots+k\alpha_{k-1}\frac{dy}{dt}\Big)=\end{equation*}\begin{equation*}
=\frac{1}{x^{k+1}}\Bif(\frac{d^{k+1}y}{dt^{k+1}}+(\alpha_1-k)\frac{d^ky}{dt^k}+\dots+k\alpha_{k-1}\frac{dy}{dt}\Big)
\end{equation*}
