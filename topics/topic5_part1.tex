...
\begin{proof}
	\ \\
	Пусть $ u = t + \tau: \dfrac{d(\vec \varphi(t + \tau))}{dt} = | t + \tau = u | = \dfrac{d \vec \varphi(u)}{du} \dfrac{du}{dt} = \dfrac{d \vec \varphi(u)}{du} = f(\varphi(u)) = f(\varphi(t + \tau))$, т.е. $\varphi(t + \tau)$ -- решение.
\end{proof}
...
\begin{proof}
	...
	Рассмотрим $ \vec \chi (t) = \vec \psi (t + (t_2 - t_1)) $, согласно предыдущей теореме $ \vec \chi(t) $ тоже является решением \eqref{avt_sys}, причём $ \vec \chi (t_1) \overset{\text{по постр.}}{=\joinrel=\joinrel=} x_0 = \vec \psi (t_2) = \vec \varphi (t_1) \Rightarrow$ По основной теореме $ \vec \varphi (t) \equiv \vec \chi(t) \defeq \vec \psi(t + (t_2 - t_1)) \Rightarrow $ траектории $ \vec \varphi (t) $ и $ \vec \psi(t) $ совпали.
\end{proof}
